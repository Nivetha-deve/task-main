...
const AddCategoryForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          if (editingCategory) {
            ...
            if (response.data && response.data.main_cat_id) {
              dispatch(updateCategory({
                id: tempId,
                newCatId: response.data.main_cat_id,
                newCatName: formData.cat_name
              }));
            }
            setSubmissionStatus(`Success: ${response.data.message}`);
    ...
    useEffect(() => {
      if (submissionStatus) {
        const timer = setTimeout(() => setSubmissionStatus(''), 5000);
            ...
            <tbody>
              {categories.length > 0 ? (
                categories.map((category, index) => (
                  <tr key={category.main_cat_id || `temp-${index}`}>
                    <td>{category.cat_name}</td>
                    <td>
                      <button onClick={() => handleEdit(category)}>Edit</button>
                    </td>
